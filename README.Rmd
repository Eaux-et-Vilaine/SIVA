...
On peut installer le package SIVA à partir de [GitHub](https://github.com/)
avec:
...
Attention pour l'instant le statut de SIVA est private. Contacter Cédric pour
les droits. 
...
`mainpass`, `hostmysql`,`pwdmysql`,`umysql` dans Rprofile.site, soit les charger
à partir d'un .yml.
Dans SIVA shiny les variables sont définies à l'aide de

```r
 get_golem_config("dbnamemysql.")
```
 la variable est lue dans inst/app/golem-config.yml  L'utilisateur mysql a des
droits en lecture seulement.

```{r chargement, eval = FALSE}
if (interactive()){
...
Pour plus d'exemples sur le chargement des données tapper
