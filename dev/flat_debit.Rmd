...
#' "canal manning", "canal ifsw (noye)" ou "canal ifsw (libre)".	
#' @export
...
```{r function-debit_vannes_volets}
...
#' Briand et Woimant (2015)
#' CHANGEMENT 2016 la référence devient niveaumerb et niveauVilaineb.
#' 
#' @return
#' 
#' @export
debit_vannes_volets <- function(){
    
}
```
  
```{r example-debit_vannes_volets}
debit_total()
```
  
```{r tests-debit_vannes_volets}
test_that("debit_vannes_volets works", {
  expect_true(inherits(debit_vannes_volets, "function")) 
})
```

# Debit de l'ensemble du barrage : debit_total
    
Cette fonction inclue le débit des vannes, des volets, des siphons, de l'écluse et de la passe.    
