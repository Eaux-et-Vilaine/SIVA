...
Utilisation de bilansiva pour extraire les débits du barrage. Le tableau dans 
cette forme peut être utilisé directement dans les fonctions de débit.
...
```{r tests-bilansiva_debit}
test_that("bilansiva-debit works", {
  expect_true(inherits(bilansiva_debit, "function")) 
})
```
...
# Traitement des données en sortie de Bilan = traitement_siva
...
test_that("traitement_siva works", {
  expect_true(inherits(traitement_siva, "function")) 
})
```
