...
assign("archive_IAV", envir_stacomi)
#if (interactive()){
...
ggplot()+ geom_line(aes(x=horodate, y=niveau, col=source), data=niveaux) 
ggplot()+  geom_line(aes(x=horodate, y=Q, col=vanne), data=debits_vannes)+ geom_line(aes(x=horodate, y=Q, col=volet), data=debits_volets)
...
```{r graphes niveaux armchart}
#install.packages("rAmCharts")
library("rAmCharts")
debit_barrage %>% rename(horodate = HoroDate) %>%
  amTimeSeries(
    'horodate',
