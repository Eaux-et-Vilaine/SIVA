  ...
  as.data.frame(
    Q12345 %>%dplyr::select(
      ...
      dplyr::summarize(
        ...
        vol_volet_vanne_bar = sum(tot_vol_vannes,  tot_vol_volet,  na.rm = TRUE),
        debit_moyen_cran = sum(debit_moyen_cran*600, na.rm=TRUE)/(24 * 60 * 60)
      ) %>%
      dplyr::mutate(
        debit_moyen_recalcule = vol_recalc/(24 * 60 * 60),
...
Q2j <- bind_cols(date, bind_cols(volets, vannes)) %>%
  ...
  inner_join(Qj,  by="date") %>% 
  mutate(
         volume_total_barQ = select(., contains(c("vol_vanne_barQ", "vol_volet_barQ",  "vol_passe", "vol_siphon",
              "vol_ecluse"))) %>% rowSums(na.rm =TRUE),
         debit_vanne_barQ = vol_vanne_barQ / 86400,
         debit_volet_barQ = vol_volet_barQ / 86400,
         debit_total_barQ = volume_total_barQ/86400
         )
...
* Débit moyen vol_bar : en partant des totaliseurs de volume au barrage,  on calcule un débit journalier.
...
Q2j %>% select(date|  starts_with("debit")) %>% select(-debit_volet_barQ,-debit_vanne_barQ) %>% 
  ...
  geom_line(aes(x=date,y=Q, col=source), alpha=0.5) 
