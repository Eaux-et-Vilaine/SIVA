...
plotly_volume_jour <- function(QV){
  ...
  V <-  QV %>% select(date |  starts_with("vol")) %>%
      select(-c(vol_bar,vol_volet,vol_vanne)) %>%  # mauvaises valeurs
      select(-c(vol_recalcule,volume_total_barQ)) %>%
      pivot_longer(
      ...
      mutate(vol=case_when(vol/10^6 < 0.01 ~ round(vol/10^6,3),
  ...
  QV2 <- inner_join(V,Q, by = c("date","source"))
  ...
  g <- ggplot(QV2) + geom_col(aes(x=date, y= vol, fill = source, customdata = date), position="stack") +
      #ylab(expression(paste(Volume~10^6~m^3))) +
      ylab("Volume en millions de m3") +
      scale_fill_manual(values=c("vanne"=bleu_EVf,
              "volet"=turquoise_EV,
              "ecluse"=jaune_EV,
              "passe"=orange_EV,
              "siphon"= bleu_clair_EV)) +
      geom_text(aes(x=date,y=vol,label=Qtxt),size=1.8,col="turquoise")+
      theme_light()
